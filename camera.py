...
class CAMERA(QWidget):
    ...
    def __init__(self,cam='choose',confFile='confCameras.ini',**kwds):
        '''
        Parameters
        ----------
...
#        print(' path conf camera',self.confpath)
    ...
    def openID(self):
        ...
        '''
        self.ccdName=self.conf.value(self.nbcam+"/name")
    ...
    def openCam(self):
        ...
        if self.nbcam=="choose": # create menu widget with all camera present 
            ...
            for groups in self.groups:
                self.groupsName.append(self.conf.value(groups+"/name"))
    ...
    def setup(self):  
            ...
            """
            self.setWindowTitle( self.cameraType+"   " + self.ccdName)
    ...
    #        self.camNameLabel.setStyleSheet('')
            ...
            self.vbox1.addWidget(self.camNameLabel)
            ...
            self.vbox1.setSizeConstraint(QtGui.QLayout.SetFixedSize)
            self.vbox1.setContentsMargins(0, 0, 0, 10)
            # vbox1.addSpacing(0)
            # vbox1.addStretch(1)
            self.widgetControl=QWidget(self)
            
            self.widgetControl.setLayout(self.vbox1)
            ...
            self.vboxSup=QHBoxLayout()
            self.widgetSup=QWidget(self)
            self.widgetSup.setLayout(self.vboxSup)
            self.dockSup=QDockWidget(self)
            self.dockSup.setWidget(self.widgetSup)
            ...
            self.dockSup.setTitleBarWidget(QWidget())
            ...
            if self.separate==True:
                ...
                if self.aff=='left':
                    ...
                    self.visualisation.addDockWidget(Qt.LeftDockWidgetArea,self.dockSup)
                else:
                    ...
                    self.visualisation.addDockWidget(Qt.RightDockWidgetArea,self.dockSup)
            else:
            #self.dockControl.setFeatures(QDockWidget.DockWidgetMovable)
                ...
                self.visualisation.addDockWidget(Qt.TopDockWidgetArea,self.dockSup)
...
if __name__ == "__main__":       
    ...
    e = CAMERA(cam='cam15',fft='off',meas='on',affLight=True,aff='right',separate=True,multi=False,toolBarOn=False)#,confpath=path  )
    e.show()
