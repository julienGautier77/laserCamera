...
from imgSourceLight2circles import CameraAcqD # affiche le centre de masse
from baslerVisionLight import CAMERABASLERACQ
import numpy as np
import pyqtgraph as pg
from scipy import ndimage
from laserbeamsize import basic_beam_size
from pypylon import pylon

class CAMMOT(QMainWindow) :
    ...
    def __init__(self,name=None,motOn=False,motLat=None,motorTypeName0=None, motVert=None,motorTypeName1=None,nomWin='',nomTilt='',unit=1,jogValue=1,visuGauche=False,parent=None):
        ...
        if self.camType == 'basler':
            #print('Basler ccd')
            self.cam=CAMERABASLERACQ(name=name)
        elif self.camType =='imgSource':
            #print('imagingSource')
            self.cam=CameraAcqD(name=name)
        else :
...
#            self.ro1.setPos([self.xec-(int(self.dx)/4),self.yec-int((self.dy)/4)])
            ...
            if self.maxx<30 :
                ...
                break
            if self.deltaY<3 and self.deltaX<3:
                break
            if abs(self.deltaX)>2.5:
...
if __name__ == "__main__":
    ...
    e = CAMMOT(name="cam12",motOn=True,visuGauche=True)#,motLat='NF_Lat',motorTypeName0='NewFocus', motVert='NF_Vert',motorTypeName1='NewFocus',nomWin='Tilts')
    e.show()
